"""
cleanops.py - DeckOps installer for CleanOps (Call of Duty: Black Ops III)

Downloads the CleanOps d3d11.dll from GitHub and drops it into the BO3
install directory. Sets the Wine DLL override launch option so the DLL
loads correctly under Proton.

CleanOps is a lightweight mod client for Black Ops III that improves
performance and fixes various issues. Installation is a single DLL —
no exe replacement, no config patching.
