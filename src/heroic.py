...
def _write_lcd_launch_wrapper(game_key: str, ge_proton_version: str,
    ...
    script = f'''#!/bin/bash
# DeckOps LCD Plutonium launcher (via Heroic)
...
LOG_DIR="$HOME/DeckOps-Nightly/logs"
LOG_FILE="$LOG_DIR/install.log"
mkdir -p "$LOG_DIR" 2>/dev/null || LOG_FILE=/dev/null

log() {{
    printf '[%s] [lcd_launch %s] %s\\n' \\
        "$(date '+%Y-%m-%d %H:%M:%S')" "$GAME_KEY" "$1" \\
...
if is_heroic_running; then
    ...
    if [ "$detected" -eq 0 ]; then
        log "WARNING Heroic wake-up timed out after 20s; firing URL anyway"
    fi
