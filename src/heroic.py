...
# Per-game launcher wrapper scripts. Steam shortcuts point at these.
# Path A (current): wrapper invokes Proton directly against Heroic's
# shared default prefix, mirroring the OLED _write_wrapper pattern from
# plutonium.py. No flatpak chain, no heroic:// URL, no Heroic at runtime.
...
def _write_lcd_launch_wrapper(game_key: str, ge_proton_version: str,
                               steam_root: str = None) -> str:
    """
    Write a per-game shell wrapper that launches Plutonium directly via
    Proton against Heroic's shared default Wine prefix.

    This is the LCD counterpart to plutonium.py's _write_wrapper (OLED).
    The wrapper structure is intentionally identical to OLED's online-mode
    wrapper:

        #!/bin/bash
        export STEAM_COMPAT_DATA_PATH="<prefix>"
        export STEAM_COMPAT_CLIENT_INSTALL_PATH="<steam_root>"
        exec "<proton>" run "<launcher>" "plutonium://play/<key>"

    The only difference vs OLED is the prefix path: OLED targets a Steam
    compatdata appid prefix, LCD targets ~/Games/Heroic/Prefixes/default
    (the prefix Heroic created during the bootstrap login flow). Same
    GE-Proton runtime, same launcher exe, same protocol URL, same env --
    just a different prefix dir.

    The user must have completed the Heroic bootstrap login flow before
    these wrappers will work, since they depend on plutonium-launcher-win32.exe
    being present inside Heroic's prefix.
    ...
    # Resolve Steam root. Falls back to the standard SteamOS path.
    if not steam_root:
        try:
            import config as _cfg
            steam_root = _cfg.load().get("steam_root")
        except Exception:
            steam_root = None
    if not steam_root:
        steam_root = os.path.expanduser("~/.local/share/Steam")

    # Resolve Proton binary from the GE-Proton version we were given.
    proton_bin = os.path.join(
        steam_root, "compatibilitytools.d", ge_proton_version, "proton"
    )

    # Plutonium launcher lives inside Heroic's shared prefix, populated
    # by Heroic during the bootstrap login flow.
    ...
    script = (
        "#!/bin/bash\n"
        "# DeckOps LCD Plutonium launcher (direct Proton invocation)\n"
        "# Auto-generated by heroic.py. Do not edit by hand.\n"
        "#\n"
        "# Mirrors plutonium.py _write_wrapper (OLED) but targets Heroic's\n"
        "# shared default Wine prefix instead of a Steam compatdata prefix.\n"
        "# No Heroic at runtime -- the wrapper invokes Proton directly.\n"
        "set -u\n"
        "\n"
        f'export STEAM_COMPAT_DATA_PATH="{HEROIC_DEFAULT_WINE_PREFIX}"\n'
        f'export STEAM_COMPAT_CLIENT_INSTALL_PATH="{steam_root}"\n'
        f'exec "{proton_bin}" run "{launcher_exe}" "{plut_url}"\n'
    )
...
def _create_heroic_steam_shortcut(game_key: str, on_progress=None):
    """
    Create a non-Steam shortcut that launches an LCD Plutonium game via
    a per-game wrapper script (see _write_lcd_launch_wrapper).

    The wrapper invokes Proton directly against Heroic's shared default
    Wine prefix. Heroic is only used for the one-time bootstrap login
    flow -- not at game launch time.
    """
    def prog(msg):
        if on_progress:
            on_progress(msg)
    ...
    # Resolve GE-Proton version (same fallback chain as setup_heroic_game)
    try:
        import config as _cfg
        ge_version = _cfg.get_ge_proton_version()
    except Exception:
    ...
    # Per-game wrapper script that runs Proton against Heroic's prefix
    wrapper_path = _write_lcd_launch_wrapper(game_key, ge_version)
